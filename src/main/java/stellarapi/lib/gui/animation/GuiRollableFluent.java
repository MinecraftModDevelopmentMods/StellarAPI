...
/**
 * Rollable GUI with support for intermediate positions, for cases i.e.
 * dragging. Note that the position only include the element to roll.
 */
public class GuiRollableFluent implements IGuiElementType<IRollableFluentController> {
	...
	public void keyTyped(char eventChar, int eventKey) {
		mover.keyTyped(eventChar, eventKey);
	}
		...
		public boolean doesStartAnimation() {
			if (rollState != controller.rollState()) {
				this.duration = (int) Math
						.ceil(Math.abs(controller.rollState() - rollState) / controller.rollRatePerTick());
				rollState = controller.rollState();
				return true;
			} else
				return false;
