...
public final class CelestialCollectionManager {
	...
	/**
	 * Finds the celestial object in certain range near certain position.
	 * <p>
	 * Note that in this method the default ordering of collection is prior to
	 * the distance to the position.
	 * 
	 * @param pos
	 *            the horizontal spherical position
	 * @param radius
	 *            the angular radius of the search range
	 * @return the object near the position in the range, or <code>null</code>
	 *         if there is no celestial object within the range
	 */
	public ICelestialObject findObjectInRange(SpCoord pos, double radius) {
		return this.findObjectInRange(pos, radius, Predicates.<ICelestialCollection> alwaysTrue(),
	 ...
	 * @param radius
	 *            the angular radius of the search range
	 * @param collectionChecker
	 *            the checker to filter collections of objects
	 * @param objectChecker
	 *            the checker to filter objects
	 * @return the object near the position in the range, or <code>null</code>
	 *         if there is no celestial object within the range
	 */
	public ICelestialObject findObjectInRange(SpCoord pos, double radius,
			...
			for (ICelestialObject object : objectSet)
				if (objectChecker.apply(object)) {
					if (foundObject == null)
						...
						foundObject = collection.getNearerObject(pos, foundObject, object);
				}
	 ...
	 * Finds the celestial object in certain range near certain position.
	 * <p>
	 * Note that in this method the ordering of collection is prior to the
	 ...
	 *            the angular radius of the search range
	 * @param ordering
	 *            the ordering to determine the order of collections
	 * @param collectionChecker
	 *            the checker to filter collections of objects
	 * @param objectChecker
	 *            the checker to filter objects
	 ...
	 *         if there is no celestial object within the range
	 */
	public ICelestialObject findObjectInRange(SpCoord pos, double radius, Ordering<ICelestialCollection> ordering,
			Predicate<ICelestialCollection> collectionChecker, Predicate<ICelestialObject> objectChecker) {
	 ...
	 * 
	 * @param pos
	 *            the horizontal spherical position of the center of the search
	 *            range
	 * @param radius
	 *            the angular radius of the search range
	 ...
	 */
	public Set<ICelestialObject> findAllObjectsInRange(SpCoord pos, double radius,
			Predicate<ICelestialCollection> checker) {
