...
/**
 * Fired to define the {@link Ordering} of the light sources, and default
 * {@link Ordering} of celestial collections.
 * <p>
 * Note that the default ordering of light sources is defined by standard
 * magnitude, and that of celestial collections is defined by priority.
 */
public class SortCelestialsEvent extends PerWorldEvent {
	...
	public SortCelestialsEvent(World world, Ordering<ICelestialCollection> defaultOrderingCC,
			List<ICelestialCollection> collections, Map<IEffectorType, List<ICelestialObject>> mapEffectors) {
		super(world);
		...
		for (Map.Entry<IEffectorType, List<ICelestialObject>> entry : mapEffectors.entrySet()) {
			effectorSources.create(entry.getKey(), ImmutableList.copyOf(entry.getValue()));
			orderingEffectors.create(entry.getKey(), entry.getKey().getOrderingFor(entry.getValue()));
		}
	}
	...
	public void setEffectorOrdering(IEffectorType type, Ordering<ICelestialObject> ordering) {
		orderingEffectors.create(type, ordering);
	}
